...
def train_model(
    ...
    for data in track(train_loader, description=f"Run train", disable=True):
        ...
        if use_amp:
            ...
            # 在裁剪前，必须显式取消梯度的缩放 (Unscale)
            # 这样 clip_grad_norm_ 面对的才是真实的、物理意义上的梯度值
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(pooler.parameters(), max_norm=max_grad_norm)
...
def datasets(sets="common"):
    ...
    for i in range(len(datasets)):
        ...
        if datasets[i] in ["FRANKENSTEIN"]:
            use_node_attr = True
        from grit.rrwp import add_full_rrwp
