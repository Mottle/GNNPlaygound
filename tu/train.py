...
from utils.perf_counter import get_time_sync, measure_time
...
class RUMWrapper(nn.Module):
    def __init__(self, in_channels: int, hid_channels: int, rum_model: RUMModel):
        super().__init__()
        self.in_channels = in_channels
        self.rum_model = rum_model
        self.encoder = nn.Linear(in_channels, hid_channels)

    def forward(self, data):
        h = self.encoder(data.x)
        wk_h, loss = self.rum_model(data, h)
        h = wk_h.mean(0)
...
def train_model(
    pooler, classifier, train_loader, optimizer, criterion, device, use_amp=False
):
    pooler.train()
    classifier.train()
...
def test_model(pooler, classifier, test_loader, criterion, device, use_amp=False):
    ...
    with torch.no_grad():
        for data in track(
            test_loader, description=f"Run test batch: {all_batches}", disable=True
        ):
            if data.x is None or data.x.size(1) <= 0:
                data.x = torch.ones((data.num_nodes, 1))
            data = data.to(device)
...
def run_fold(dataset, loader, current_fold: int, config: BenchmarkConfig):
    ...
    for epoch in track(range(epochs), description=f"{log_prefix} 训练 {dataset}"):
        ...
        val_loss, val_acc = test_model(
            model, classifier, val_loader, criterion, run_device, use_amp=use_amp
        )
        test_loss, test_acc = test_model(
            model, classifier, test_loader, criterion, run_device, use_amp=use_amp
        )
...
def run_k_fold4dataset(dataset, config: BenchmarkConfig):
    ...
    for fold, (train_idx, test_idx) in track(
        enumerate(kfold_dataset), total=config.kfold, description="k-fold"
    ):
        ...
        if config.catch_error:
            try:
                train_result, test_result = run_fold(
                    dataset,
...
def process_results(results: list[BenchmarkResult]):
    ...
    return (
        (all_mean, all_std),
        (last_mean, last_std),
        (last_10_mean, last_10_std),
        (last_50_mean, last_50_std),
    )
...
def datasets(sets="common"):
    ...
    for i in range(len(datasets)):
        ...
        if datasets[i] in ["FRANKENSTEIN"]:
            use_node_attr = True
        yield TUDataset(
            root="./tu/dataset/", name=datasets[i], use_node_attr=use_node_attr
        )
...
def run(config: BenchmarkConfig):
    ...
    for dataset in track(datasets(config.sets), description="All Datasets"):
        ...
        save_result(
            ...
            f"./stgnn/result_fin/{config.model}.txt",
