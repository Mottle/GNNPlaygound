...
def mask_nodes(x, batch, mask_ratio=0.15):
    ...
    nodes_per_graph = torch.zeros(batch.max().item() + 1, device=device).scatter_add_(0, batch, ones)
    ...
    # 计算每个 batch 的起始索引偏移
    cum_nodes = torch.cat([torch.tensor([0], device=device), nodes_per_graph.cumsum(0)[:-1]])
    ...
    for i, (start_idx, count) in enumerate(zip(cum_nodes.long(), num_to_mask_per_graph)):
        if count > 0:
